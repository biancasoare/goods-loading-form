...
{
    $field = form_input($name, 'password', $elementData, $params);
...
function form_choice($name, $multiple = false, $expanded = false, Array $initData = [], $elementData = null, $params = '')
{
    ...
    if (true === $expanded){ // => putem avea un radio sau un checkbox
        ...
        foreach ($initData as $k => $v)
        {
            ...
            $return .= '<input type="'.$type.'" id="'.$k.'" name="'.$name.'" value="'.$v.'"';

            if ($params) $return .= " $params";

            if ((is_array($elementData) && in_array($v, $elementData)) || (is_string($elementData) && ($k == $elementData)) ) $return .= ' checked';
